...
export async function addComment1(code, postId, comment) {
  // comment: { author, text } 또는 string (하위호환)
  await updateDoc(doc(db, 'rooms', code, 'step1Posts', postId), { comments: arrayUnion(comment) })
}

export async function deleteComment1(code, postId, comment) {
  // arrayRemove with objects can fail due to deep-equality edge cases
  // Use filter + setDoc approach for reliability
  const ref = doc(db, 'rooms', code, 'step1Posts', postId)
  const snap = await getDoc(ref)
  if (!snap.exists()) return
  const comments = snap.data().comments || []
  const isObj = typeof comment === 'object' && comment !== null
  const updated = comments.filter(item => {
    if (!isObj || typeof item !== 'object' || !item) return true
    return !(item.author === comment.author && item.text === comment.text)
  })
...
export async function addComment4(code, postId, comment) {
  await updateDoc(doc(db, 'rooms', code, 'step4Posts', postId), { comments: arrayUnion(comment) })
}

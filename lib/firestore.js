...
// ── Step 1 Comments (subcollection - 삭제 100% 보장) ─────────────────────
// post document의 comments 배열 대신 별도 subcollection 사용
// deleteDoc(commentId) 방식으로 array 비교 문제 완전 우회

export async function addComment1(code, postId, author, text) {
  await addDoc(collection(db, 'rooms', code, 'step1Comments'), {
    postId, author, text, createdAt: serverTimestamp()
  })
}

export async function deleteComment1(code, commentId) {
  // commentId = subcollection 문서 ID → deleteDoc으로 100% 신뢰 삭제
  await deleteDoc(doc(db, 'rooms', code, 'step1Comments', commentId))
}

export function subscribeStep1Comments(code, cb) {
  const q = query(collection(db, 'rooms', code, 'step1Comments'), orderBy('createdAt', 'asc'))
  return onSnapshot(q, snap => cb(snap.docs.map(d => ({ commentId: d.id, ...d.data() }))))
}

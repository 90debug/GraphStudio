    ...
    <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}
      ...
      <div className="p-5">
        ...
        {post.noteTexts?.length > 0 && (
          <div className="mb-3">
            <div style={{ fontSize: isMobile ? 10 : 12, fontWeight:900, color:'#94a3b8', textTransform:'uppercase', letterSpacing:'.5px', marginBottom:8 }}>알 수 있는 사실</div>
            <div className="flex flex-col gap-1.5">
              {post.noteTexts.map((t, i) => (
                <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:8, fontSize: isMobile ? 12 : 14, color:'#475569', lineHeight:1.6 }}>
        ...
        {post.ps && (
          <div className="mb-3">
            <div style={{ fontSize: isMobile ? 10 : 12, fontWeight:900, color:'#94a3b8', textTransform:'uppercase', letterSpacing:'.5px', marginBottom:6 }}>탐구 문제 해결</div>
            <div style={{ fontSize: isMobile ? 12 : 14, color:'#475569', lineHeight:1.6, padding:'8px 12px', borderRadius:12, background:'#f8fafc', border:'1px solid #f1f5f9', whiteSpace:'pre-line' }}>{post.ps}</div>
          </div>
        )}
        <div className="flex items-center gap-2 mt-2">
          ...
          <span style={{ fontSize: isMobile ? 10 : 12, fontWeight:700, color:'#94a3b8' }}>{doneCount}/{CHECKLIST.length}</span>
        </div>
      </div>
            ...
            <div style={{ borderTop:'1px solid #f1f5f9', padding:'10px 14px 12px', background:'#fafafa', borderRadius:'0 0 20px 20px' }}>
              <div className="space-y-2">
                ...
                <div style={{ display:'flex', gap:8, paddingTop:4 }}>
                  ...
                  <button onClick={submitCmt} style={{ padding:'8px 14px', borderRadius:999, background:'#5B41EB', color:'#fff', border:'none', fontSize: isMobile ? 11 : 13, fontWeight:700, cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap', flexShrink:0, minHeight:36 }}>등록</button>
                </div>
              </div>
            </div>
          </motion.div>
...
function WorkPanel({ code, user, items, dataTable, chartConfig }) {
  ...
  const labelStyle = { fontSize: isMobile ? 12 : 14, fontWeight:700, color:'#64748B', marginBottom:6 }
  const guideStyle = { fontSize: isMobile ? 12 : 14, fontWeight:400, color:'#94A3B8', marginBottom:7 }
    ...
    <div style={{ display:'flex', flexDirection:'column', overflow:'hidden', flex:1 }}>
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', padding:14 }}>
        ...
        <div style={{ background:'#EEEEF3', border:'1px solid #e2e3e5', borderRadius:12, padding:12, marginBottom:12 }}>
          <div style={{ fontSize: isMobile ? 12 : 14, fontWeight:700, color:'#5B41EB', marginBottom:6 }}>{chartConfig.title||'완성된 그래프'}</div>
          {hasData ? (
            <div
          ...
          <button onClick={doLoadCanvas} disabled={loadingImg} style={{ marginTop:8, width:'100%', padding:'6px', borderRadius:7, border:'1px solid #e2e3e5', background:'#fff', color:'#5B41EB', fontSize: isMobile ? 11 : 13, fontWeight:700, cursor:'pointer', fontFamily:'inherit', minHeight:36 }}>
        ...
        <div style={{ marginBottom:12 }}>
          ...
          {notes.length>0 ? (
            <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
              {notes.map((n,i)=>(
                <div key={n.id} style={{ display:'flex', alignItems:'flex-start', gap:5, padding:'6px 10px', borderRadius:8, fontSize: isMobile ? 12 : 14, fontWeight:500, lineHeight:1.5, background:CHART_COLORS[i%CHART_COLORS.length]+'12', border:`1px solid ${CHART_COLORS[i%CHART_COLORS.length]}25`, position:'relative' }}>
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <textarea value={ps} onChange={e=>setPs(e.target.value)}
            ...
            rows={4}
            style={{ width:'100%', padding:'9px 10px', borderRadius:8, border:'1px solid #F1F5F9', fontSize: isMobile ? 12 : 14, background:'#F8FAFC', outline:'none', resize:'none', lineHeight:1.6, fontFamily:'inherit' }}
        ...
        <div style={{ marginBottom:12 }}>
          ...
          {CHECKLIST.map((item,i)=>(
            <label key={i} style={{ display:'flex', alignItems:'flex-start', gap:8, marginBottom:8, cursor:'pointer' }}>
              ...
              <span style={{ fontSize: isMobile ? 12 : 14, lineHeight:1.55, color:checks[i]?'#5B41EB':'#475569', fontWeight:checks[i]?700:400, transition:'all .15s', flex:1 }}>{item}</span>
            </label>
          ))}
          <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:6 }}>
            ...
            <span style={{ fontSize: isMobile ? 11 : 13, fontWeight:700, color:'#5B41EB', whiteSpace:'nowrap' }}>{doneCount}/{CHECKLIST.length} 완료</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding:12, flexShrink:0, borderTop:'1px solid #F1F5F9' }}>
        <button onClick={doShare} disabled={sharing||!canShare} style={{
          ...
          color:(sharing||!canShare)?'#94A3B8':'#fff', border:'none', fontSize: isMobile ? 13 : 15, fontWeight:700,
    ...
    <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', padding:isMobile?'12px 12px 16px':'14px 16px 20px', background:'#F3F4F8', position:'relative' }}>
      {posts4&&posts4.length>0 ? (
        ...
        <div style={{ textAlign:'center', padding:'40px 20px', color:'#94A3B8' }}>
          ...
          <div style={{ fontSize: isMobile ? 15 : 16, fontWeight:700, color:'#64748B', marginBottom:6 }}>아직 공유된 해석이 없어요.</div>
          <div style={{ fontSize: isMobile ? 13 : 15 }}>해석을 작성하고<br/><b>보드에 공유하기</b>를 눌러 보세요!</div>
        </div>
      )}
    </div>
  )
      ...
      <header className="px-6 h-14 bg-white/70 backdrop-blur-lg border-b border-slate-100 flex items-center justify-between sticky top-0 z-40" style={{ flexShrink:0 }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p style={{ fontSize: isMobile ? 12 : 14, color:'#8A949E', fontWeight:700, marginTop:4 }}>그래프 해석하기</p>
          </div>
        </div>
          ...
          <div style={{ display:'flex', borderBottom:'1px solid #e2e3e5', flexShrink:0, background:'#fff' }}>
            {[['work','내 해석 작성'],['board','공유 보드']].map(([id,label])=>(
              <button key={id} onClick={()=>setMobileTab(id)} style={{
                flex:1, padding:'10px 0', fontSize: isMobile ? 13 : 15, fontWeight:mobileTab===id?800:600,
        ...
        <div style={{ flex:1, display:'flex', overflow:'hidden' }}>
          <div style={{
            ...
            <div style={{ padding:'10px 12px', borderBottom:'1px solid #F1F5F9', display:'flex', alignItems:'center', gap:8, flexShrink:0, background:'#F8FAFC' }}>
              {!collapsed && <span style={{ fontSize: isMobile ? 13 : 15, fontWeight:700, color:'#1E293B', flex:1, whiteSpace:'nowrap' }}>내 해석 작성</span>}

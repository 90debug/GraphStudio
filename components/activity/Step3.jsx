    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{display:'flex',gap:7}}>
          {[['draw','✏️ 직접 그리기','#C97DE8'],['auto','📊 자동 그래프','#5BBF7A']].map(([mode,label,clr])=>(
            <button key={mode} type="button" onClick={()=>onDrawMode(mode)} style={{
              ...
              padding:'9px 20px',minHeight:44,borderRadius:999,fontSize: isMobile ? 13 : 14,fontWeight:700,
      ...
      <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',padding: isMobile ? '10px 12px 20px' : '12px 14px 20px'}}>
        {drawMode==='auto'?(
          <Sec>
            <Lbl mt={0}>📊 그래프 종류</Lbl>
            <div style={{display:'flex',gap:7,flexWrap: isMobile ? 'nowrap' : 'wrap',marginBottom:14,overflowX: isMobile ? 'auto' : 'visible',paddingBottom: isMobile ? 4 : 0}}>
              {CHART_TYPES.map(c=>(
